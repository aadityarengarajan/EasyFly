...
#/- to fetch aircraft image for live tracking
...
CLIENT_VERSION = 'build-2206-202106-4951-13-oss'
...
async def devbackup(ctx):
    ...
    if str(author.id) in AUTHORIZED_DEVELOPERS_LIST:
        ...
        logs(f'\n[{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")}] @{author.name} requested developer backup data, data was given\n')
        ...
        await author.send(file=logsfile)
        ...
        logs(f'\n[{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")}] @{author.name} requested developer backup data, but was unauthorized\n')
...
11=PBN%2FA1B1C1D1O1S1+DOF/{str(dt.now().strftime('%y%m%d'))} RMK%2FTCAS {rem} GENERATED BY EASYFLY FOR SIMULATION ONLY&
...
''')
        ...
        try:
            ...
            await member.send("Use the `wxr` command (`>wxr` or `//wxr`) to get live weather, and `rwy` to get active runways of your departure and arrival airports :wink:")
        ...
        try:
            ...
            await member.send(formatted_atsfpl)
        ...
        try:
            ...
            try:
                ...
                await member.send(f"Sorry Captain {pic}, we faced an error while generating your pre-file links. Perhaps your flight plan was too long.\nHowever, your flight briefing and plan have been sent above, enjoy your flight captain, Good Day!")
           ...
           (flight.get("network") not in POSTED) == True:
            ...
            try:
                ...
                if flight["network"] == "VATSIM":
                    embed.set_thumbnail(url="https://vatsim-forums.nyc3.digitaloceanspaces.com/monthly_2020_08/Vatsim-social_icon.thumb.png.e9bdf49928c9bd5327f08245a68d8304.png")

                elif flight["network"] == "IVAO":
                    embed.set_thumbnail(url="https://hr.ivao.aero/images/New_IVAO_Branding/logo_s_blue.png")

                else:
                    embed.set_thumbnail(url="https://res.cloudinary.com/crunchbase-production/image/upload/c_lpad,f_auto,q_auto:eco/zvv1nhs6kopbhhutdvcm")
